...
_DEFAULT_CONFIG_PATH = Path("configs/enrichment.yaml")


def load_enrichment_config(config_path: Path | None = None) -> EnrichmentConfig:
    """Load and validate the enrichment config from a YAML file.

    Args:
        config_path: Path to the YAML config. Defaults to configs/enrichment.yaml.

    Returns:
        Validated EnrichmentConfig model.
