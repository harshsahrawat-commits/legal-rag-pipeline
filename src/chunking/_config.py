...
_DEFAULT_CONFIG_PATH = Path("configs/chunking.yaml")


def load_chunking_config(config_path: Path | None = None) -> ChunkingConfig:
    """Load and validate the chunking config from a YAML file.

    Args:
        config_path: Path to the YAML config. Defaults to configs/chunking.yaml.

    Returns:
        Validated ChunkingConfig model.
